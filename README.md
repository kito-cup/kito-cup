<div align="center">

# Hi, I'm Maksyta

### Future Robotics Engineer

<img src="https://skillicons.dev/icons?i=cpp,git,linux,arduino,vscode" />

<br>

> 🎓 Future student of Mechatronics & Robotics  
> 💻 Learning C++, Git and Linux  
> 🤖 Building Arduino  
> 🎬 Video Editor (Premiere Pro & After Effects)

</div>

---

## 🧑‍💻 About Me

```cpp
class Max {
public:
    string goal = "Become a Robotics Engineer";
    string currentFocus = "C++, Linux, Arduino";
    string hobby = "Video Editing";
    string dream = "Build useful technology";
};
```
<img width="480" height="360" alt="gif (1)" src="https://github.com/user-attachments/assets/29c202a8-8ecd-4e16-b83a-5e275eaa6c77" />
## 📈 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=kito-cup&show_icons=true)




---

## 🔥 Current Projects

🟡 C++ Calculator
🟡 ATM Simulator
🟡 Traffic Light Simulator
🟡 Smart Home System

---

<div align="center">

</div>
